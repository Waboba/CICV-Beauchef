function [results] = geckoOptKnockGurobi(ecModel, biomassIdx, targetIdx, nKnockouts)
    ...
    % Ya incluyen los valores  -(MW/kcat)*v 
    ...
    % En vez de utilizar la dualidad completa, vamos a forzar que la
    % biomasa no sea cero o esté cerca del óptimo (esto logra un mejor
    % resultado cuando los modelos son muy grandes).
    row_bio = sparse(1, totalVars);
    row_bio(biomassIdx) = 1;
    model.A = [model.A; row_bio];
    model.rhs = [model.rhs; 0.01]; % Valor mínimo de crecimiento (ajustable)
    model.sense = [model.sense; '>'];
